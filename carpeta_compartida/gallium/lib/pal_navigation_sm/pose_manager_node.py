...
class PoseSaver:
  ...
  def __init__(self, map_frame, base_frame):
    ...
    rospy.Subscriber("robot_pose", PoseWithCovarianceStamped, self._pose_callback, queue_size=1)
  ...
  def _save_pose(self, pose, covariance):
    ...
    try:
      ...
      shutil.copyfile(self._filename, self._filename + '.bkp')
...
def main():
  rospy.init_node("pose_manager_node")

  pm = PoseManager()
  pm.checkInitialPose()
  
  rospy.set_param("~check_pose", "true")
