...
do
    ...
    if [ -f $MOUNT_DIR/.disk/info ]; then
        if [ "`grep Lubuntu $MOUNT_DIR/.disk/info`" != "" ]; then
            MOUNTED="yes"
            break
        elif [ "`grep Ubuntu $MOUNT_DIR/.disk/info`" != "" ]; then
            MOUNTED="yes"
            break
        elif [ "`grep ^$PAL_SOFTWARE_ISO_PREFIX[0-9] $MOUNT_DIR/.disk/info`" != "" ]; then
            MOUNTED="yes"
            break
        else
